...
import { ChevronDown, ChevronUp, Plus, X, CheckSquare, ListTodo } from "lucide-react";
...
const PackingChecklist = ({ checklist, onChange }: PackingChecklistProps) => {
  ...
  const [expandedCategories, setExpandedCategories] = useState<string[]>(["Essentials", "Clothing", "Technology", "Toiletries", "Kyoto Specific"]);
  ...
  const toggleChecklistItem = (id: string) => {
    const updatedItems = checklist.map((item) => (item.id === id ? { ...item, checked: !item.checked } : item));
  ...
  const getCategoryProgress = (category: string) => {
    ...
    return {
      ...
      percent: categoryItems.length > 0 ? Math.round((checkedItems.length / categoryItems.length) * 100) : 0,
    };
  };
      ...
      <CardContent className="pt-0">
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            ...
            />
            <select value={newItemCategory} onChange={(e) => setNewItemCategory(e.target.value)} className="p-2 border rounded-md">
          ...
          <div className="space-y-2">
            {categories.map((category) => {
              ...
              return (
                <Collapsible key={category} open={isExpanded} className="border rounded-md overflow-hidden">
                  <CollapsibleTrigger asChild>
                    <div className="flex justify-between items-center p-3 cursor-pointer hover:bg-muted" onClick={() => toggleCategory(category)}>
                      ...
                      {checklist
                        ...
                        .map((item) => (
                          <div key={item.id} className="flex items-center justify-between py-1">
                            <div className="flex items-center gap-2">
                              <Checkbox checked={item.checked} onCheckedChange={() => toggleChecklistItem(item.id)} id={item.id} />
                              <label htmlFor={item.id} className={`text-sm ${item.checked ? "line-through text-muted-foreground" : ""}`}>
                                {item.name}
                              </label>
                            </div>
                            <Button variant="ghost" size="icon" className="h-6 w-6 rounded-full opacity-60 hover:opacity-100" onClick={() => removeChecklistItem(item.id)}>
                              <X className="h-3 w-3" />
                            </Button>
                          </div>
                        ))}
